...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'])) {
    ...
    // 1️⃣ Update abstract status to Approved
    $update = $conn->prepare("UPDATE abstracts SET status = 'Approved' WHERE id = ?");
    if (!$update) {
    ...
    if ($update->execute()) {
        ...
        // 2️⃣ Get uploader user_id + title for the notification message
        ...
        if ($result && $row = $result->fetch_assoc()) {
            $uploader_id = $row['user_id']; // ✅ Correctly placed here
            $title = $row['title'];

            // 3️⃣ Insert notification for that uploader only
            $msg = "Your abstract titled '{$title}' has been approved and added to the library.";
            ...
            if ($notif->execute()) {
                echo "<script>
                        alert('Abstract approved and user notified successfully!');
